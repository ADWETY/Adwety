...
exports.createTransfer = asyncHandler(async (req, res) => { const body = req.validated.body; const row = await StoreTransfer.create({ number: await nextRetailNumber(StoreTransfer, 'TR', 'transfer'), date: normalizeDate(body.date), fromWarehouseId: body.fromWarehouseId || body.from_warehouse_id, toWarehouseId: body.toWarehouseId || body.to_warehouse_id, items: body.items.map((x) => ({ productId: x.productId || x.product_id, qty: x.qty })), notes: body.notes || '', status: body.status || 'completed', createdBy: req.authUser?._id }); await applyTransferEffects(row); await row.populate(['fromWarehouseId', 'toWarehouseId', 'items.productId']); return success(res, transferDto(row), 'Transfer created', 201); });
