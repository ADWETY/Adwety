...
const dashboardPasswordResetRoles = ['admin', 'pharmacist'];
const resetAccountNotFoundMessage = 'البريد الإلكتروني غير موجود في قاعدة البيانات أو لا يملك صلاحية دخول للوحة التحكم.';
...
exports.forgotPassword = asyncHandler(async (req, res) => {
  ...
  // Dashboard reset must be explicit: do not create a fake OTP request for unknown, inactive,
  // or non-dashboard accounts. The frontend must only move to the OTP step after this endpoint
  // returns a real request_id from createOtpRequest().
  const user = await User.findOne({
    email,
    isActive: { $ne: false },
    role: { $in: dashboardPasswordResetRoles }
  }).select('_id email role fullName isActive');

  if (!user) {
    throw new AppError(resetAccountNotFoundMessage, 404, { code: 'RESET_ACCOUNT_NOT_FOUND' });
  }

  // Check SMTP/dev OTP readiness only after confirming the dashboard account exists.
  assertOtpDeliveryReady();

  let created;
  try {
    created = await createOtpRequest({ purpose: 'password_reset', user, request: req });
  } catch (error) {
    await systemLog({
      type: 'error',
      ...
      metadata: { provider: 'smtp', error: error.message, code: error.details?.code || null },
      ip: req.ip
    });
    throw new AppError('Unable to send the password reset code. Please check SMTP settings and try again.', 503, { code: 'OTP_DELIVERY_FAILED' });
  }
  ...
  const response = {
    ...
    otp_token: requestId,
    expires_in_minutes: created.expiresInMinutes || env.otpTtlMinutes,
    delivery: { channel: created.channel || 'email', destination: created.destination || maskEmail(email) },
    email_queued: true
  };
  ...
  return success(res, response, 'Password reset code sent');
});
