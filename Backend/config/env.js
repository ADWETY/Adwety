...
function buildMailFrom() {
  const explicit = firstClean(process.env.SMTP_FROM, process.env.EMAIL_FROM);
  if (explicit) return explicit;
  const email = firstClean(process.env.MAIL_FROM_EMAIL, process.env.SMTP_USER, process.env.MAIL_USERNAME);
  if (!email) return '';
  const name = firstClean(process.env.MAIL_FROM_NAME, 'ADWETY');
  return `${name} <${email}>`;
}
...
module.exports = {
  ...
  mailFromName: firstClean(process.env.MAIL_FROM_NAME, 'ADWETY'),
  mailFromEmail: firstClean(process.env.MAIL_FROM_EMAIL, process.env.SMTP_USER, process.env.MAIL_USERNAME),
  smtpHost: firstClean(process.env.SMTP_HOST, process.env.MAIL_HOST),
  smtpPort: number(process.env.SMTP_PORT || process.env.MAIL_PORT, 587, 1, 65535),
  smtpSecure: bool(process.env.SMTP_SECURE, String(process.env.SMTP_PORT || process.env.MAIL_PORT || '') === '465'),
  smtpUser: firstClean(process.env.SMTP_USER, process.env.MAIL_USERNAME),
  smtpPass: firstSecret('SMTP_PASS', 'MAIL_PASSWORD'),
  smtpFrom: buildMailFrom(),
  ...
  otpSilentDeliveryFailure: bool(process.env.OTP_SILENT_DELIVERY_FAILURE, nodeEnv === 'production'),
