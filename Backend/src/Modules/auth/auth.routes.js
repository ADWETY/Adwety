...
const { authLimiter, authEmailLimiter, otpEmailLimiter } = require('../../middleware/security');
...
router.post('/register', authLimiter, authEmailLimiter, validate(registerSchema), controller.register);
router.post('/register/verify-otp', authLimiter, validate(verifyOtpSchema), controller.verifyRegisterOtp);
router.post('/login', authLimiter, authEmailLimiter, validate(loginSchema), controller.login);
