...
const retailTabs = [
  ...
  { to: '/warehouses', en: 'Warehouses', ar: 'المخازن' },
  { to: '/transfers', en: 'Transfers', ar: 'التحويلات' },
...
function PageTitle({ icon: Icon, title, description, action }) {
  return (
    <>
    <section className="card p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-start gap-4">
          {Icon ? <span className="rounded-2xl border border-cyan-200 bg-cyan-50 p-3 text-cyan-700 dark:border-cyan-400/30 dark:bg-cyan-500/10 dark:text-cyan-200"><Icon className="h-6 w-6" /></span> : null}
          <div>
...
export function RetailDashboardPage() {
  ...
  return <div className="space-y-6">
    <PageTitle icon={BarChart3} title={B('MATGR Retail Dashboard', 'لوحة MATGR للبيع والمخازن')} description={B('POS, barcode search, invoice details/actions, products, categories, warehouses, returns, treasury and advanced reports are available.', 'نقطة البيع والبحث بالباركود وإجراءات الفواتير والأصناف والتصنيفات والمخازن والمرتجعات والخزينة والتقارير متاحة الآن.')} action={resetButton(reset, L('reset'))} />
    ...
    <section className="grid gap-6 xl:grid-cols-3">
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <Link className="btn-secondary justify-start gap-2" to="/transfers"><ArrowLeftRight className="h-4 w-4" />{L('warehouseTransfer')}</Link>
...
export function TreasuryPage() {
  ...
  return <div className="space-y-6"><PageTitle icon={WalletCards} title={B('Treasury / Cashbox', 'الخزينة')} description={B('Track treasury balance, manual income, expenses, invoice cash movements and returns impact with MATGR-style date/type/warehouse filters.', 'متابعة رصيد الخزينة والإيرادات والمصروفات وحركات الفواتير والمرتجعات مع فلاتر التاريخ والنوع والمخزن.')} />
    ...
    <section className="card p-5"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-7"><TextInput label={L('search')} value={query} onChange={setQuery} /><TextInput label={L('fromDate')} type="date" value={fromDate} onChange={setFromDate} /><TextInput label={L('toDate')} type="date" value={toDate} onChange={setToDate} /><SelectInput label={L('type')} value={typeFilter} onChange={(value) => { setTypeFilter(value); setSummaryFilter(value === 'income' ? 'income' : value === 'expense' ? 'expense' : 'net'); }}><option value="all">{L('all')}</option><option value="income">{L('income')}</option><option value="expense">{L('expense')}</option></SelectInput><SelectInput label={L('warehouse')} value={warehouseFilter} onChange={setWarehouseFilter}><option value="all">{L('all')}</option>{data.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</SelectInput><div className="flex items-end"><button className="btn-secondary w-full" type="button" onClick={clearFilters}>{L('resetFilters')}</button></div><div className="flex items-end gap-2"><button className="btn-secondary w-full gap-2" type="button" onClick={() => { exportToCsv('treasury.csv', autoRows); toast.success(L('exported')); }}><Download className="h-4 w-4" />{L('export')}</button><button className="btn-secondary w-full gap-2" type="button" onClick={() => printElementById('treasury-table', L('treasuryReport'))}><Printer className="h-4 w-4" />{L('print')}</button></div></div></section>
