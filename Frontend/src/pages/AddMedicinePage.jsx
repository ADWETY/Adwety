...
export default function AddMedicinePage() {
  ...
  useEffect(() => { async function load() { try { const result = await getJson('/pharmacies'); const list = extractArray(result, demoPharmacies); setPharmacies(list); const pharmacyId = params.get('pharmacy_id') || list[0]?.id || ''; setForm((current) => ({ ...current, pharmacy_id: pharmacyId })); } catch (_error) { const pharmacyId = params.get('pharmacy_id') || demoPharmacies[0]?.id || ''; setForm((current) => ({ ...current, pharmacy_id: pharmacyId })); } } load(); }, [params]);
