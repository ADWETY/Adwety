...
import { extractPharmacyDetails, getJson } from '../lib/api';
...
export default function PharmacyDetailsPage() {
  ...
  useEffect(() => { async function load() { try { const result = await getJson(`/pharmacies/${id}`); setDetails(extractPharmacyDetails(result)); setError(''); } catch (loadError) { setDetails(null); setError(loadError.message || t('toast.failed')); } finally { setLoading(false); } } load(); }, [id, t]);
  if (loading) return <div className="card p-6 text-sm text-muted">{t('app.loading')}</div>;
  if (!details) return <div className="space-y-4">{error ? <div className="rounded-3xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-100">{error}</div> : null}<EmptyState title={t('common.noData')} /></div>;
  const { pharmacy, inventory, stats } = details;
  return <div className="space-y-6">
    <section className="card p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-3xl font-semibold text-primary">{pharmacy.name}</h3><p className="mt-2 text-sm text-muted">{pharmacy.address}</p><p className="mt-2 text-sm text-muted">{pharmacy.phone} · {pharmacy.email}</p></div><div className="flex flex-wrap gap-3"><span className={`badge ${statusTone(pharmacy.status)}`}>{t(`common.${pharmacy.status}`, pharmacy.status)}</span><span className="badge border-slate-200 bg-white text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">{t('common.rating')}: {pharmacy.rating}</span></div></div><div className="mt-5 grid gap-4 md:grid-cols-2"><div className="sub-card p-4"><p className="text-sm text-muted">{t('common.workingHours')}</p><p className="mt-2 font-semibold text-primary">{pharmacy.working_hours || '—'}</p></div><div className="sub-card p-4"><p className="text-sm text-muted">{t('common.googleMapsUrl')}</p><a className="mt-2 inline-flex font-semibold text-cyan-700 underline dark:text-cyan-200" href={pharmacy.google_maps_url || `https://www.google.com/maps?q=${pharmacy.latitude},${pharmacy.longitude}`} target="_blank" rel="noreferrer">{t('actions.openMaps')}</a></div></div></section>
