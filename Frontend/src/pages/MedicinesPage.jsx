...
import { Download, Eye, Pencil, Plus, Printer, Search, Sparkles, Trash2, X } from 'lucide-react';
...
const emptyFilters = { q: '', smart: '', category: '', pharmacy: '', stock: '', form: '', minPrice: '', maxPrice: '', sortBy: 'name' };

function medicineSmartFilter(item, query) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return true;
  const quantity = Number(item.quantity || 0);
  const price = Number(item.price || 0);
  const text = `${item.name || ''} ${item.category || ''} ${item.pharmacy_name || ''} ${item.form || ''} ${item.strength || ''} ${item.description || ''} ${quantity} ${price}`.toLowerCase();
  if (/(low|منخفض|قليل|ناقص)/.test(q)) return stockStatus(quantity) === 'low_stock';
  if (/(out|zero|empty|صفر|نفد|غير متوفر|منتهي)/.test(q)) return stockStatus(quantity) === 'out_of_stock';
  if (/(available|متاح|متوفر)/.test(q)) return stockStatus(quantity) === 'in_stock';
  if (/(expensive|غالي|عالي السعر)/.test(q)) return price >= 100;
  if (/(cheap|رخيص|منخفض السعر)/.test(q)) return price > 0 && price < 50;
...
export default function MedicinesPage() {
  ...
  async function loadData() {
    try {
      ...
      setRows(extractArray(medicines));
      setPharmacies(extractArray(pharmacyResult));
      setError('');
    } catch (loadError) {
      setRows([]); setPharmacies([]); setError(loadError.message || t('toast.failed'));
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((item) => {
      ...
      if (!medicineSmartFilter(item, filters.smart)) return false;
  ...
  async function deleteRow() {
    ...
    try {
      await deleteJson(`/medicines/${item.id}${item.inventory_id ? `?inventory_id=${item.inventory_id}` : ''}`);
      await loadData();
  ...
  return <div className="space-y-6">
    ...
    <section className="card p-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div><label className="label">{t('actions.search')}</label><div className="relative"><Search className="pointer-events-none absolute start-4 top-3.5 h-4 w-4 text-soft" /><input className="input ps-11" value={filters.q} onChange={(e) => updateFilter('q', e.target.value)} placeholder={t('forms.medicineNamePlaceholder')} /></div></div>
        <div><label className="label">{t('medicines.smartFilter')}</label><div className="relative"><Sparkles className="pointer-events-none absolute start-4 top-3.5 h-4 w-4 text-cyan-500" /><input className="input ps-11" value={filters.smart} onChange={(e) => updateFilter('smart', e.target.value)} placeholder={t('medicines.smartFilterPlaceholder')} /></div></div>
